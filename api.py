...
class PetFriends:
    ...
    def update_pet_info(self, auth_key: json, pet_id: str, name: str, animal_type: str, age: int) -> json:
        """Метод отправляет запрос на сервер о обновлении данных питомwа по указанному ID и
    ...
    def add_photo_for_pet(self, auth_key: json, pet_id: str, pet_photo: str) -> json:
        """Метод отправляет (постит) фото питомца и возвращает статус
        запроса на сервер и результат в формате JSON с данными добавленного питомца"""

        headers = {"auth_key": auth_key['key']}
        file_format =''
        if "jpeg" in pet_photo:
            file_format = 'jpeg'
        elif 'jpg' in pet_photo:
            file_format = 'jpg'
        elif 'png' in pet_photo:
            file_format = 'png'

        file = {"pet_photo": (pet_photo, open(pet_photo, 'rb'), f'image/{file_format}')}
